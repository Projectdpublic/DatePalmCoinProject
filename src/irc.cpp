...
void ThreadIRCSeed(void* parg)
{
    ...
    RenameThread("DatePalmCoin-ircseed");
...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #DatePalmCoinTEST2\r");
            Send(hSocket, "WHO #DatePalmCoinTEST2\r");
        } else {
            // randomly join #DatePalmCoin00-#DatePalmCoin05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #DatePalmCoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #DatePalmCoin%02d\r", channel_number).c_str());
        }
